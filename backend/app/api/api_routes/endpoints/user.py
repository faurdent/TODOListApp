...
async def verify_account(verification_user_code: str, db: AsyncSession = Depends(get_db)):
    ...
    verified_user_obj = await user.activate_account(db, verification_code_obj.user_id)
    return verified_user_obj


@router.post("/send-verification-email")
async def send_verification_email(
        user_info: UserSchema, background_tasks: BackgroundTasks, db: AsyncSession = Depends(get_db)
):
